...
const setupData = {
  classes:   [],  // { name, color }
  people:    [],  // string[]
  timeSlots: []   // { start, end, type }
};

const SLOT_TYPE_LABELS = {
  class: 'Class', recess: 'Recess', lunch: 'Lunch', break: 'Break'
};
...
function renderClassesList() {
  ...
  container.innerHTML = setupData.classes.map((cls, i) => `
    <div class="item-tag">
      <span class="color-dot" style="background:${cls.color}"></span>
      <span>${cls.name}</span>
      <button class="remove-item" onclick="removeClass(${i})">&times;</button>
    </div>
  `).join('');
}
...
function renderPeopleList() {
  ...
  container.innerHTML = setupData.people.map((person, i) => `
    <div class="item-tag">
      <span>${person}</span>
      <button class="remove-item" onclick="removePerson(${i})">&times;</button>
    </div>
  `).join('');
}
...
function addTimeSlot() {
  ...
  let endTime   = '10:00';
  if (setupData.timeSlots.length > 0) {
    const last = setupData.timeSlots[setupData.timeSlots.length - 1];
    startTime = last.end;
...
function renderTimeSlots() {
  ...
  container.innerHTML = setupData.timeSlots.map((slot, i) => {
    const type = slot.type || 'class';
    return `
      <div class="time-slot-row">
        <span>Period ${i + 1}:</span>
        <input type="time" value="${slot.start}" onchange="updateTimeSlot(${i}, 'start', this.value)">
        <span>to</span>
        <input type="time" value="${slot.end}" onchange="updateTimeSlot(${i}, 'end', this.value)">
        <select onchange="updateTimeSlot(${i}, 'type', this.value)" class="slot-type-select">
        ...
        <button class="remove-slot" onclick="removeTimeSlot(${i})">&times;</button>
      </div>
    `;
  }).join('');
}
...
function populateReview() {
  ...
  document.getElementById('reviewPeople').innerHTML = setupData.people.map(p => `
    <span class="review-item">${p}</span>
  `).join('');
  ...
  document.getElementById('reviewSlots').innerHTML = setupData.timeSlots.map(slot => {
    ...
    let prefix;
    if (type === 'class') { periodIndex++; prefix = `Period ${periodIndex}`; }
    else { prefix = label; }
